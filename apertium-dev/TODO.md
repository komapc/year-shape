...
## 🔍 Data Investigation
- [ ] **Investigate Missing "cenaristo":** Ido Wikipedia has an article [Cenaristo](https://io.wikipedia.org/wiki/Cenaristo), but it is missing from our dictionary.
  - Check if it exists in the raw Wikipedia XML dumps.
  - Investigate why it wasn't extracted into `source_io_wikipedia.json`.
  - Current status: Found as an Epo translation `scenaristo` in BERT embeddings, but missing as an Ido lemma in all sources.

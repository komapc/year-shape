...
def generate_monodix(input_file: Path, output_file: Path, min_confidence: float = 0.0):
    ...
    if pardefs_file.exists():
        ...
        try:
            ...
            if not pardefs.findall("./pardef[@n='num_ord_regex']"):
                ...
                ET.SubElement(p_ord2, 'l').text = '(ma|esma)'
    ...
    # Add generic number entries to the main section using literal regexes
    # This ensures lt-comp treats them as top-level patterns

    # 1. Ordinals (16-ma, 16ma, 7esma)
    num_ord_regex_entry = ET.SubElement(section, 'e')
    ET.SubElement(num_ord_regex_entry, 're').text = '[0-9]+(-ma|-esma|ma|esma)'
    p_ord = ET.SubElement(num_ord_regex_entry, 'p')
    ET.SubElement(p_ord, 'l').text = ""
    r_ord = ET.SubElement(p_ord, 'r')
    r_ord.text = "-a"
    ET.SubElement(r_ord, 's', {'n': 'num'})
    ET.SubElement(r_ord, 's', {'n': 'ord'})

    # 2. Cardinals (123, 1.23, 4 803)
    num_regex_entry = ET.SubElement(section, 'e')
    ET.SubElement(num_regex_entry, 're').text = '[0-9]+([ .,][0-9]+)*'
    p_num = ET.SubElement(num_regex_entry, 'p')
    ET.SubElement(p_num, 'l').text = ""
    r_num = ET.SubElement(p_num, 'r')
    ET.SubElement(r_num, 's', {'n': 'num'})
    ET.SubElement(r_num, 's', {'n': 'ciph'})
    ET.SubElement(r_num, 's', {'n': 'sp'})
    ET.SubElement(r_num, 's', {'n': 'nom'})
